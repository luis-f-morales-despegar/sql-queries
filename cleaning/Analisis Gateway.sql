...
------------------------------------------------------------------------------------------------------------------------------------------
 ...
 --- Power BI "Analysis nube" (tabla "Data") - Yas ---
...
where gateway = 'EXP'
  ...
  and parent_channel not in (
      select distinct parent_channel
      from bt_detail
      where gateway != 'EXP'
  )
  and producto_original not in (
      select distinct producto_original
      from bt_detail
      where gateway != 'EXP'
  )
  and Viaje not in (
      select distinct Viaje
      from bt_detail
      where gateway != 'EXP'
  )
group by 
   ...
   with bt_detail as (
    select 
        ...
        max(case 
                ...
                when (tx.channel like '%hoteldo%') then ca.total    
                else (ca.gross_booking - ca.perceptions - ca.tax_pais) 
            end) as gb_usd
    from data.analytics.bi_transactional_fact_products pr 
...
where gateway = 'EXP'
  and not exists (
      select 1
      from bt_detail as other
      where other.gateway != 'EXP'
        and other.pais = bt_detail.pais
        and other.parent_channel = bt_detail.parent_channel
        and other.producto_original = bt_detail.producto_original
        and other.Viaje = bt_detail.Viaje
  )
group by 
    pais,
    parent_channel,
