...
bt_detail as (
    			...
    			--		fh.product_status,
                    ...
                    where fh.gestion_date >= DATE('2025-01-01')
...
------------------------------------------------------------------------------------------------------------------------------------------
 ...
 --- Reconocimiento de Ingreso (apertura agencia) - Mail Diario Excel -- NR y FVM ajustados por gradiente 20250304
...
with bt_detail as (
  ...
  --                  ,sum(pr.currency_hedge_usd + pr.currency_hedge_air_usd) as hedge
            ...
            where fv.recognition_date between DATE('2025-01-01') and DATE('2025-12-31')
...
--- Carte de Agencias - Query del Sales Tracking (Dash) de Vic
