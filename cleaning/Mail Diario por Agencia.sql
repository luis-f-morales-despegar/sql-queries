...
--- Mail Diario Excel ---  Se añade fix_fvm 2025-06-05
...
bt_detail as (
    			...
    			--		fh.product_status,
	                 ...
	                 sum(((pnl.margin_net_usd + pnl.variable_charges_without_mkt_usd + pnl.financial_result_usd 
                           ...
                           / if(pnl.b2b_gradient_margin = '1', 1, cast(pnl.b2b_gradient_margin as decimal(2,2)) ) -- quitar gradiente para obtener bruto
                         )
...
select 
    ...
    --   ,max(p.created) as created
       ,max(year(p.created)) AS anio_created
       ,max(month(p.created)) AS mes_created
