...
SELECT 
    ...
    sum(t.financial_result_usd) as financial_result_usd,
    SUM(t.ccp) AS ccp,
...
FROM (
    SELECT
        ...
        cob.merchant_id,
        sum(cob.amount) as amount,
        sum(cob.total_transaction_amount) as total_transaction_amount,
        sum(s.gestion_gb) as gestion_gb,
        SUM(s.gestion_gb * s.confirmation_gradient) as gb_gradiente,
        sum(m.coi_usd) as coi_usd,
        sum(m.financial_result_usd) as financial_result_usd,
        sum(m.coi_interest_usd) as coi_interest_usd,
...
SUM(total_registros)  AS registros,
        ...
        SUM(((-m.coi_usd + coi_interest_usd + m.financial_result_usd) / IF(m.b2b_gradient_margin = '1', 1, CAST(m.b2b_gradient_margin AS DECIMAL(2,2)))) * cob.amount / cob.total_transaction_amount) AS coi_net,
...
INNER JOIN analytics.bi_pnlop_fact_current_model m 
    ...
    WHERE 
        ...
        and cob.merchant_ownership = 'OWN'
        and cob.type = 'Tarjeta de Crédito'
        AND s.country_code IN ('MX', 'BR')
...
WHERE
    t.orders > 0
   and t.transaction_code = 47529282100  --90859277400 --15537279000
    and t.product_line = 'Hoteles'
...
INNER JOIN analytics.bi_pnlop_fact_current_model m 
        ...
        LEFT JOIN (
            SELECT 
                ...
                SUM(amount) OVER (PARTITION BY transaction_code) AS total_transaction_amount
            FROM analytics.bi_transactional_fact_collections
            WHERE reservation_year_month >= DATE '2024-10-01'
         --   AND state = 'OK'
            ) cob 
            ON s.transaction_code = CAST(cob.transaction_code AS BIGINT)
    WHERE 
