...
SELECT 
    ...
    MAX(
      IF(tx.channel = 'expedia', 'veronica.odetti@hoteldo.com', ca.director)
    )                              as director,
...
GROUP BY
    ...
    -- ✅ Opción A) Devolver SOLO el total de filas resultantes (un único número)
--    Envuelve tu SELECT agrupado como subconsulta y haz COUNT(*)
...
base_rows AS (
  SELECT 
      ...
      MAX(tx.parent_channel)         AS parent_channel,
      MAX(sp.estado_producto)        AS estado_producto,
      MAX( IF(tx.channel = 'expedia', 'expedia', tx.partner_data_id) ) AS partner_id,
      MAX(tx.line_of_business)       AS lob,  
      MAX(tx.brand)                  AS brand,
      MAX( IF(tx.channel = 'expedia', 'veronica.odetti@hoteldo.com', ca.director) ) as director,
      MAX(pr.gateway)                AS gateway,
      MAX(pr.provider_code)          AS provider,
      MAX(pr.hotel_despegar_id)      AS hotelid,
      MAX(pr.effective_rate)         AS tarifaefectiva,
 ...
 --     MAX(t.rule_name)               AS rule_name,
  ...
  WHERE 1=1
    ...
    AND (
          ca.director = 'veronica.odetti@hoteldo.com'
       OR tx.channel = 'expedia'
       OR ( (ca.director = 'TBD' OR ca.director IS NULL)
            AND tx.site IN ('Usa', 'Panama', 'España') )
    )
  GROUP BY 1,2,3
)
...
-- ✅ Opción B) Agregar una columna con el total de filas (ventana) SIN perder el detalle
--    Útil si quieres ver cada fila + el total global repetido en cada una
...
WITH bo_tpc AS (
  SELECT 
    ...
    MAX(p.net_commission_partner * p.conversion_rate) AS tpc_usd,
    MAX(CAST(p.cancelled AS DATE)) AS cancelled,
    MAX(CASE WHEN COALESCE(p.status, '') = '' THEN 'ACTIVE' ELSE p.status END) AS bo_status
  FROM data.lake.channels_bo_product p
