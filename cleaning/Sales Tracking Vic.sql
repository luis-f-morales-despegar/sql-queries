--- Query de Ventas del Sales Tracking dash Vic ----
...
,fact_sales as (
    ...
    where s.partition_period >= '2023-01'
    and s.gestion_date >= date'2023-01-01'
...
ultimate_facts as (
  select 
    ...
    join analytics.bi_transactional_fact_products pr on pr.product_id = fv.product_id and pr.reservation_year_month >= date'2023-01-01'
    left join analytics.bi_transactional_fact_charges c on pr.product_id = c.product_id and c.reservation_year_month >= date'2023-01-01'
...
--    where pr.hotel_despegar_id = '2867398'
    where fv.checkin_date >= date'2024-01-01'
