with productos_detalle as (select 
...
/***** Resultados Financieros *****/    
    ...
    ,sum(b.van_collection_usd) - sum (b.collection_amount_usd) as diferencia_cobro
    ,sum(b.payment_amount_usd) as payment_amount_usd
    ,sum(b.van_payment_usd) as van_payment_usd
    ,sum (b.payment_amount_usd) - sum(b.van_payment_usd) as diferencia_pago
...
join data.analytics.bi_pnlop_fact_current_model b on p.product_id=b.product_id and b.date_reservation_year_month > '2023-01-01'
...
and t.confirmation_date BETWEEN DATE '2025-01-01' AND date_add('day', -1, current_date)
...
agentes as (
        select 
            a.agent_code
            ,max(upper(a.first_name || ' ' || a.last_name)) as agent_name		
        from data.lake.ch_bo_partner_agent a 
        group by 1
),
pnl as (         
        select b.transaction_code as tx_code  
            ,b.product_id   
            ,sum(b.payment_amount_usd) as gb     
            ,sum(b.margin_variable_net_usd) as margen_var     
            ,sum(b.revenue_sharing_usd) as comisionb_2b2c       
            ,-sum(b.ccp_usd) as ccp
            ,-sum(b.coi_usd) as coi 
        from data.analytics.bi_pnlop_fact_current_model b
        where b.date_reservation_year_month >= '2022-01'
        and date(b.date_reservation) BETWEEN DATE '2025-01-01' AND date_add('day', -1, current_date)
       -- t.confirmation_date BETWEEN DATE '2025-01-01' AND (CURRENT_DATE - INTERVAL '1 day')
        group by 1,2
),
cobros as(        
        select 
            distinct(cob.transaction_code) as transaction_code,         	
            ARRAY_JOIN(ARRAY_AGG(DISTINCT cob.type), ',') as metodo_cobro,    
            ARRAY_JOIN(ARRAY_AGG(DISTINCT cob.installments), ',') as cuotas,
            max(cob.installments) as cuotas_max,
            max(cob.card_brand) as tarjeta_marca,        	  
            max(cob.card_bank) as tarjeta_banco,        	   
            max(cob.state) as resultadocobro,        	  
            max(cob.coupon_type_name) as cupon_tipo,	 
            max(cob.channel_id) as tarjeta_channel_id,
            max(cob.merchant_ownership) as merchant_ownership,
            max(cob.merchant_id) as merchant
        from analytics.bi_transactional_fact_collections cob	       
        where cob.reservation_year_month >= date('2021-07-01')	      
        and cob.state = 'OK'	       
        Group by 1
),
tipo_cambio as (       
        select 
            s.transaction_id,        
            avg(p.conversion_rate) as tipo_cambio      
        from data.lake.channels_bo_sale s         
        inner join data.lake.channels_bo_product p on s.id = p.sale_id        
        Group by 1
)
/*****************************************************/
select   
        pr.transaction_code as tx_code,  
        pr.product_id,
        max(cobros.metodo_cobro) as metodo_cobro,   
        max(cobros.cuotas) as cuotas,  
        max(cobros.cuotas_max) as cuotas_max,
        max(cobros.tarjeta_marca) as tarjeta_marca, 
        max(cobros.tarjeta_banco) as tarjeta_banco,   
        max(cobros.resultadocobro) as resultado_cobro,   
        max(tarjeta_channel_id) as channel_cobro,
        max(merchant_ownership) as merchant_ownership,
        max(merchant) as merchant,
        sum(pnl.coi) as coi,
        sum(pnl.ccp) as ccp
from data.analytics.bi_transactional_fact_products pr 
join data.analytics.bi_transactional_fact_transactions tx on tx.transaction_code = pr.transaction_code and tx.reservation_year_month >= date('2021-07-01')
left join data.analytics.bi_transactional_fact_charges c on pr.product_id = c.product_id and c.reservation_year_month >= date('2021-07-01')
left join tipo_cambio cr on cr.transaction_id = cast(tx.transaction_code as bigint)
left join cobros on cast(cobros.transaction_code as bigint) = cast(tx.transaction_code as bigint)
left join pnl on cast(pnl.tx_code as bigint) = cast(tx.transaction_code as bigint) and cast(pnl.product_id as varchar) = cast(pr.product_id as varchar)
where tx.confirmation_date BETWEEN DATE '2025-01-01' AND date_add('day', -1, current_date)
and pr.reservation_year_month >= date('2023-01-01')
and tx.line_of_business in ('B2B2C')
and tx.country_code = 'BR'
--and tx.partner_data_id ='Sicredi'
group by 1,2
)
select * from productos_detalle pd left join cuotas on pd.trx_code = cast(cuotas.tx_code as bigint) and pd.product_id = cuotas.product_id