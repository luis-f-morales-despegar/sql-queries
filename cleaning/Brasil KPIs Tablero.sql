...
SELECT 
    ...
    CAST(MAX(fv.gestion_date) AS DATE) AS gestion_date,
...
WHERE 1=1
  ...
  AND pr.reservation_year_month < CURRENT_DATE
  AND tx.reservation_date     > DATE('2023-12-31')
  AND tx.reservation_date     < DATE('2024-09-01')
  AND tx.parent_channel in ('API', 'Agencias afiliadas')
  AND tx.site = 'Brasil'
GROUP BY
    tx.parent_channel
    ...
    fv.country_code,
    fv.parent_channel,
     ROUND(sum(fV.gestion_gb),0) as gb_s_gradiente,
     ROUND(sum(fv.gestion_gb * fv.confirmation_gradient),0) as gb_cgx
FROM data.analytics.bi_transactional_fact_products pr
JOIN data.analytics.bi_transactional_fact_transactions tx
 ON tx.transaction_code = pr.transaction_code
 AND tx.reservation_year_month > DATE('2023-12-31')
...
LEFT JOIN data.analytics.bi_pnlop_fact_current_model b
  ON b.product_id = pr.product_id
  AND b.date_reservation_year_month > '2024-09-01' --------------
WHERE 1=1
  AND pr.reservation_year_month >= DATE('2025-08-01')
  AND pr.reservation_year_month < DATE('2025-09-01')
--  AND fv.gestion_date > DATE '2023-12-31'
 -- AND fv.gestion_date < DATE('2024-09-01')
  --AND tx.reservation_date     > DATE('2023-12-31')
  --AND tx.reservation_date     < DATE('2024-09-01') -------------
  --AND fv.parent_channel in ('API', 'Agencias afiliadas')
  --AND fv.site = 'Brasil'
--  AND fv.country_code = 'BR'
  and tx.partner_data_id IN ('AP12907', 'AP12908', 'AP12909', 'AP12910')
   AND fv.lob_gestion IN ('stg__sales_b2bnohoteldo','stg_sales__b2bhoteldo')
    AND fv.channel NOT IN ('bestday-wl-mobile', 'affiliate-sicoob')
GROUP by
    fv.site,
    fv.country_code,
    fv.parent_channel
  ...
  WHERE fh.partition_period > '2023-12'
    AND fh.gestion_date > DATE '2023-12-31'
    AND fh.gestion_date < DATE('2024-09-01')
...
--------------------------------------------------------------------------------------
---- Sales Tracking de Vic

select * 
from lake.b2b_pbi_sales_tracking
