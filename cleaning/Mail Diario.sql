...
--- Mail Diario Excel ---  Venta Emitida ---- Se añade fix_fvm 2025-06-05 -- se corrige PY 2025-07-29
...
bt_detail as (
                    select
                     ...
                     -- fh.country_code as pais,
                        case 
                       when fh.partner_id in ('AP12142', 'AP12961', 'AP12767', 'AP12539', 'AP12792', 'AP12149', 'AP12148', 'AG00015606', 'AP13029', 'AP13030', 'AP13091', 'AP13029', 'AP13030', 'AP13104', 'AG00015611') then 'PY'
    			...
    			--		fh.transaction_code as tx_code,
                    ...
                    --    fh.channel,
                     --   fh.trip_type_code as viaje, 
    					sum(fh.gestion_gb) as gb_s_gradiente,
                        sum(fh.gestion_gb * fh.confirmation_gradient) as gb, 
                        sum(case when fh.country_code = 'BR' and fh.product not in ('Vuelos') then (pnl.net_revenues_usd - (bo.tpc_usd * if(pnl.b2b_gradient_margin = '1', 1, cast(pnl.b2b_gradient_margin as decimal(4,3)) ) ) )
                    ...
                    from ( select *
		                ...
		                where fv_prev.partition_period > '2023-12'
	                     ) fh
                    left join analytics.bi_pnlop_fact_current_model pnl on fh.product_id = pnl.product_id and pnl.date_reservation_year_month > '2023-12'
                    left join analytics.bi_transactional_fact_charges c on fh.product_id = c.product_id and c.reservation_year_month > date'2023-12-31'
                    left join analytics.bi_pnlop_fact_pricing_model pr on pr.product_id = fh.product_id and pr.date_reservation_year_month > '2023-12'
                    ...
                    where fh.gestion_date > DATE('2023-12-31')
...
--and (agent_code in ('AG00015611') or agency_code in ('AG00015611') or partner_id in ('AG00015611'))
limit 100
...
with bo_tpc as( 
            ...
            from data.lake.channels_bo_product p
            join data.lake.channels_bo_sale s on s.id = p.sale_id
            where cast(s.created as date) between date_add('month',-1,date('2023-01-01')) and date_add('month',1,CURRENT_DATE) 
            group by 1
    ),
bt_detail as (
               select
          ...
          --         	,fv.parent_channel
           --        	,fv.channel   	
                   	    ,CASE 
            when fv.partner_id in ('AP12142', 'AP12961', 'AP12767', 'AP12539', 'AP12792', 'AP12149', 'AP12148', 'AG00015606', 'AP13029', 'AP13030', 'AP13091', 'AP13029', 'AP13030') then 'PY'
  ...
  END
                    ...
                    ,sum( case when fv.country_code = 'BR' and fv.product not in ('Vuelos') then ((pnl.net_revenues_usd/if(pnl.b2b_gradient_margin = '1', 1, cast(pnl.b2b_gradient_margin as decimal(4,3)))) - (fv.tpc_fix_iva))
                          when fv.channel = 'expedia' then ((pnl.net_revenues_usd/if(pnl.b2b_gradient_margin = '1', 1, cast(pnl.b2b_gradient_margin as decimal(4,3)))) - (fv.tpc_fix_iva))    
                          else pnl.net_revenues_usd/if(pnl.b2b_gradient_margin = '1', 1, cast(pnl.b2b_gradient_margin as decimal(4,3)))
                          end
                        ) as fix_net_revenues
            ...
            where fv.recognition_date between DATE('2023-01-01') and DATE('2025-12-31')
...
where 1=1
and lob = 'B2B'
and pais_corregido in ('PA', 'US','ES','CR')
--and parent_channel_corregido = 'Agencias afiliadas'
--AND
...
bt_detail as (
    		...
    		--			fh.product_status,
    				...
    				ROUND(sum(fh.gestion_gb),2) as gb_s_gradiente,
                        ROUND(sum(fh.gestion_gb * fh.confirmation_gradient),2) as gb_gx, 
        ...
        --               ROUND(sum(pnl.net_revenues_usd),2) as net_revenues_usd_s_ajuste,
                        ROUND(sum(pnl.npv_net_usd),2) as fmv_net_usd,
                    ...
                    left join analytics.bi_pnlop_fact_current_model pnl on fh.product_id = pnl.product_id and pnl.date_reservation_year_month >= '2024-01'
                    left join analytics.bi_transactional_fact_charges c on fh.product_id = c.product_id and c.reservation_year_month >= date'2024-01-01'
                    left join analytics.bi_pnlop_fact_pricing_model pr on pr.product_id = fh.product_id and pr.date_reservation_year_month >= '2024-01'
                    left join bo_tpc bo on bo.product_id_original = fh.origin_product_id
